...
def test_send_mld_v2_without_router_alert():
    """Send MLDv2 packet without Router Alert"""
    ...
    link_local = fetch_link_local('r1-eth0')

    logger.info("Sending MLDv2 packet without Router Alert")
    cmd = f"--src_ip {link_local} --dst_ip {link_local} --record {GADDR} --iface 'h1-eth0' --count 3 --type 143"

    logger.info(cmd)

    tgen.gears["h1"].run(f"python {CWD}/../../packets/mld/mld_v2.py {cmd}")
...
def test_send_mld_v2_with_router_alert():
    """Send MLDv2 packet with Router Alert"""
    ...
    cmd = (f"--src_ip {link_local} --dst_ip {link_local} --record {GADDR_RA} "
           f"--iface 'h1-eth0' --count 3 --type 143 --enable_router_alert")
...
def test_mld_v2_interface_router_alert_packet_without_router_alert():
    """Send MLDv2 packet without Router Alert"""
    ...
    logger.info("Sending MLDv2 packet without Router Alert")
    cmd = (f"--src_ip {link_local} --dst_ip {link_local} --record {GADDR_RA_ENABLED} "
           f"--iface 'h1-eth0' --count 3 --type 143")
    tgen.gears["h1"].run(f"python {CWD}/../../packets/mld/mld_v2.py {cmd}")
...
def test_mld_v2_interface_router_alert_packet_with_router_alert():
    """Send MLDv2 packet without Router Alert"""
    ...
    logger.info("MLDv2 packet sent successfully")
...
def fetch_link_local(interface):
    tgen = get_topogen()
